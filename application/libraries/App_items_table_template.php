...
abstract class App_items_table_template
{
    ...
    public function can_update_optionable_items()
    {
        if ($this->type == 'estimate' && ! in_array($this->transaction->status, [
            5, // expired
            4, // accepted
        ])) {
            return true;
        ...
        return (bool) ($this->type == 'proposal' && ! in_array($this->transaction->status, [
            2, // declined
            3, // accepted
        ]));
    }

    public function has_optional_items()
    {
        return collect($this->items)->where('is_optional', 1)->count() > 0;
    }
    ...
    {
        $trAttributes = ' data-item-id="' . $item['id'] . '"';

        if ($this->admin_preview == true) {
            $trAttributes .= ' class="sortable" ';
        }

        $styleArray = [];

        if ($this->for === 'html') {
            if (($item['is_optional'] ?? false) && ! ($item['is_selected'] ?? false)) {
                $styleArray[] = 'opacity:0.5';
            }
        }

        if ($font_size = $this->get_pdf_font_size()) {
            $styleArray[] = 'font-size:' . $font_size . 'px';
        }

        if (! empty($styleArray)) {
            $trAttributes .= ' style="' . implode(';', $styleArray) . '"';
        }
    ...
    protected function set_all_taxes($taxes, $item)
    {
        foreach ($taxes as $tax) {
            if (($item['is_optional'] ?? false) && ! $item['is_selected']) {
                continue;
            }
